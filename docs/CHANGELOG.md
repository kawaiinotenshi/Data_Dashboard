...
### 系统稳定性修复
- WebSocket连接失败问题修复
  - 修正websocket.js中URL构造，移除/api后缀
  - 确保WebSocket正确连接到http://localhost:8081/ws
  - 实现实时数据更新功能

- 侧边栏菜单bug修复
  - 修正router/index.js中DepartmentManagement路由组件
  - 确保点击"员工管理/部门管理"时显示正确的表格视图

- 运输数据获取失败问题修复
  - 修正TransportRequestVO模型，移除非数据库字段
  - 调整OrderServiceImpl中运输任务生成逻辑
  - 确保运输数据API正常响应

- 订单数据获取错误修复
  - 恢复OrderMapper.xml中被注释的字段映射
  - 确保warehouse_id、transport_id、required_capacity字段正确映射

### LogiMind流程闭环实现
- 订单创建与仓库容量联动
  - 订单创建时自动减少仓库容量
  - 计算并更新仓库利用率
  - 当利用率>90%或<10%时发送库存预警

- 订单状态与运输任务联动
  - 订单状态变为"processing"时自动生成运输任务
  - 基于订单信息自动填充运输任务详情
  - 实现月份格式自动提取和格式化
